...
const BLOG_BASE_URL = 'https://srcmaxim.com/blog/';

const ATOM_FEED_XML = `<?xml version="1.0" encoding="UTF-8"?>
<feed xmlns="http://www.w3.org/2005/Atom" xmlns:media="http://search.yahoo.com/mrss/">
    <title>Maksym Koval</title>
    <id>https://srcmaxim.com</id>
    <icon>https://srcmaxim.com/favicon.ico</icon>
    <logo>https://srcmaxim.com/apple-touch-icon.png</logo>
    <link rel="alternate" href="https://srcmaxim.com"/>
    <link rel="self" href="https://srcmaxim.com/blog/feed.xml"/>
    <updated>%UPDATED%</updated>
    <author>
        <name>Maksym Koval</name>
    </author>
%ENTRIES%
</feed>`;
...
customElements.define('blog-generator', class BlogGenerator extends HTMLElement {
    ...
    async startProcessing(blogFolder) {
        ...
        const articlesFolder = await this.#blogFolder;
    ...
    async processArticleContent(main, path) {
        ...
        [...main.querySelectorAll('img')].map((elem) => {
            ...
            if (src.indexOf('http') !== 0) {
                elem.setAttribute('src', new URL(`${path.name}/${src}`, BLOG_BASE_URL));
            }
        });
        ...
        [...main.querySelectorAll('iframe')].map((elem) => {
            ...
            elem.replaceWith(p);
            if (src.indexOf('http') !== 0) {
                a.href = new URL(`${path.name}/${src}`, BLOG_BASE_URL);
            } else {
                a.href = src;
            }
        });
